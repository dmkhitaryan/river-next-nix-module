...
hash=$(nix-prefetch-git --url https://codeberg.org/sunn4room/bridge --rev "v$latest_tag" | jq -r '.hash')

source "$SCRIPT_DIR/../update-lib.sh"
update_src "$SCRIPT_DIR/package.nix" "$latest_tag" "$hash"

wget "https://codeberg.org/sunn4room/bridge/raw/tag/v${latest_tag}/build.zig.zon" -O "$SCRIPT_DIR/build.zig.zon"
